...
タイトルを「生命の夜 — A Night of Life」に決定。総合ブラッシュアップを **3 段階** で進行中。

- **段階1 = 完了・コミット済み**(本セッション):
...
- **段階2 = 未着手(次の本命)**: A案「系譜の夜」。選択基準を「時刻適応」から
  「血を残せている家系か(lineageVigor)」へ寄せ、家系の勢いと相続(子が親色をまとう
  フェードイン)を見せる。画面描画は基本ノータッチで安全
- **段階3 = 未着手**: B案「レジームの風」。数分周期で「層流⇄渦」が交代する内的な
  選択圧を field に導入し、進化の追走を見せる。**本実装前に sharing 係数のオフライン
  検証が必須**(全種が単一型に崩壊する地雷)
...
- 2026-06-13 (生命の夜・段階1): 総合ブラッシュアップ(本セッション、1 コミット)
...
- **段階2(本命): A案「系譜の夜」**。選択基準を時刻適応から lineageVigor
  ...
  画面描画は基本ノータッチで安全
- **段階3: B案「レジームの風」**。field に「層流⇄渦」が数分周期で交代するレジームを
  入れ、選択圧を時刻からレジーム適合へ。**着手前に headless で step を数千回回す
  sharing 係数スイープ必須**(0.8 で崩壊実測。0.6/60 が現状の安定点)
