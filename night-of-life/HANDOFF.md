...
**2026-06-18 セッション: オプション・計器表示の整理が完成**。
...
- 2026-06-18 (オプション・計器表示の整理): 本セッション、コミット ad995e2
...
### WebGL2 化 Step 2-2(最優先)

Step 2-1 で WebGL2 の土台と HDR FBO は揃った。次は**画面表現の本格強化**:

- **ブルーム(光のにじみ)**: ダウンサンプル(1/2 → 1/4)+ 分離ガウシアン + 加算合成で 5-6 パス、FBO 3 枚程度。
  Step 2-1 で立てた `programBlit` を入口に、複数 FBO を ping-pong させる。RGBA16F の HDR 値があるので、
  閾値超過(value > 1.0)分だけにじませる「明部のみブルーム」が綺麗に効くはず。
...
- **カメラは常に固定**(cameraMotion オプション自体を 2026-06-18 に撤去)。動きを全部「流れ」にして「カメラか流れか分からない」を回避(あめさん要望)。
