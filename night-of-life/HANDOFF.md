...
**2026-06-20: WebGL2 化 Step 2-2 (ブルーム) 完成 + ON/OFF 二値化**。あめさんと多観点 workflow で設計案 4 案を
比較検証し、採用案「**軽量 Bright pass + 半解像度 1 段の 9-tap 分離ガウシアン**」(設計案 C 発展形)を実装。
HDR FBO の明部のみを Rec.709 luminance で抽出して半解像度に落とし、横→縦の 2 パスでガウシアンぼかしを
かけ、blit を拡張して加算合成する構成。FBO 追加は半解像度 RGBA16F が 2 枚(ping-pong)、
シェーダー追加は BrightBlurH と BlurV の 2 本のみで最小侵襲。
**100% 超で実機にちらちら(にじみ同士の重なりで加算合成が飽和してフリッカ)** とあめさん FB を受けて、
スライダー(0..150)を **チェックボックス(ON で 50% 相当 = 0.5、OFF で 0)** に変更。
**既定 OFF で起動する opt-in**(あめさん視認確定後の二段階デプロイ予定。LP / lively / webui / Defaults すべて OFF)。
**オービット中(env.dragBoost=15)は effectiveStrength = Settings.bloomStrength /
dragBoost で 1/15 に減衰**させ、操作トリガーの「一様な呼吸」禁忌(HANDOFF 5 章)に踏まない構造分離。
**`?bloom=0` キルスイッチ**で強制 OFF、**`?bloomDebug=1`** で半解像度バッファを可視化。
**進化・物理(genome/evolution/species/field)・OBSERVATORIUM は無改修**。新遺伝子なし =
`art-evolution-v2` 据え置き。webui の Defaults snapshot に `bloomStrength` が boolean で加わったので
`art-web-settings-v1` には bloom 値も保存される(旧クライアントの数値は起動時 1 回だけ古い値で動き、
次の save() で boolean に書き換わる)。詳細は 3 章、落とし穴は 5 章。
...
### WebGL2 化 Step 2-2(ブルーム) = 完了(2026-06-20)

採用案「軽量 Bright pass + 半解像度 1 段の 9-tap 分離ガウシアン」を実装・確定。詳細は 3 章。
**ブルーム本体は完了**したので、次は以下のサブ TODO へ:

- **既定値の二段階デプロイ → 最終的にオプション撤去の検討(最優先)**:
  Settings.bloomStrength の既定値を **OFF (false) → ON (true = 0.5) へ動かす**。
  あめさんが実機 ON で放置(2-3 分)・オービット中・cameraZoom 200% + 明るさ 130% などの
  最悪条件で「画面全体の一斉発光」「白飛び大輪」が出ないことを確認したら、
  `js/lively.js` Settings.bloomStrength / `js/webui.js` Defaults.bloomStrength /
  `LivelyProperties.json` value の **3 箇所を同期更新** (false → true)。
  既存ユーザー(`art-web-settings-v1` に bloomStrength=false が保存されている)は loadSaved で
  上書きされるので影響を受けないが、新規ユーザー / Lively で初めて壁紙追加した人には ON 状態で見せる。
  次の段階で「オプション撤去 + 50% 固定」もあめさん本人の意向で検討。撤去するなら触る場所:
  (a) LivelyProperties.json から bloomStrength 項目を削除、
  (b) webui.js の CONTROLS / Defaults / STR.bloomStrength / STR.bloomUnavailable / disabled 処理を削除、
  (c) lively.js の Settings 初期値を 0.5 に変更し case 'bloomStrength' を削除、
  (d) render-gl.js の effectiveStrength は Settings.bloomStrength を直読みするので Settings 側で 0.5 固定なら無改修、
  (e) HANDOFF L390 周辺と落とし穴セクションを更新
- **被写界深度(DoF)**: 軽量フェイクで十分(z 値ベースで点サイズを膨らませて加算合成に流す = 奥が自動的ににじむ)。
  本格 CoC + bokeh は重すぎ。Step 2-1 の atmospheric perspective と接合しやすい。
- **高密度化と位相同期の波**: 粒子量上限を 7000 → 15000〜20000 へ。`MAX = 30000` まで buffer 確保済みなので
  shader 側は無改修で増やせる(`LivelyProperties.json` の max を上げるだけ)。位相同期の波は色相だけに作用させて
  「規則的明滅」(あめさん NG)を避ける。ブルームと同時に効くと「一様な呼吸」のリスクが高まるので、
  既定値を 0 → 40-60 で動かす二段階デプロイの後で着手する
- **やる時に確認**: `gl.getParameter(gl.MAX_TEXTURE_SIZE)` が 4K * dpr を満たすか / `EXT_color_buffer_half_float` が
  MSAA も対応するか(対応していれば距離ベース AA を MSAA に置き換え可能)。
...
- **強度 100% 超のちらちら → ON/OFF 化(2026-06-20)**: あめさん実機 FB「100% を超えると、
  にじみ同士が重なった時にちらちらする」を受けて、当初のスライダー(0..150)を ON/OFF のチェックボックスに
  変更(ON で内部値 0.5 = 従来の 50% スライダー相当、OFF で 0)。にじみ同士の重なりで加算合成が容易に
  飽和してフリッカに見える現象は、設計時の risk リスト「密集白飛びの拡大」の派生。スライダーに戻したい場合は
  `js/webui.js` の CONTROLS で type 'slider' に戻し min=0/max=50/step=5 にする(100% 超を許すと再びちらつく
  ので max=50 上限が安全)。lively.js の listener は boolean / number 両対応なのでスライダー値も受け取れる
- **既定値 OFF (opt-in) を外す時の二段階デプロイ**: 既存ユーザーは `art-web-settings-v1` に
  bloomStrength=false が保存されているので、`js/lively.js` の Settings 初期値(現在 0)を 0.5 に上げても
  loadSaved で上書きされて影響を受けない。Lively で初めて壁紙追加した新規ユーザーや、localStorage クリア後の
  ユーザーには新既定値が反映される。**3 ファイル(lively.js Settings / webui.js Defaults / LivelyProperties.json
  value)を同時更新しないと、片方だけ反映される不整合が出る**(LP のリスナーは webui の Defaults と
  独立に動くので、片方だけ ON にすると Web 版と Lively 版で挙動が割れる)
...
- **Canvas 2D 経路 (`?gl=0`) との視覚差**: 2D 経路はブルーム未実装(設計時点で意図的に WebGL2 限定機能)。
  ...
  1 行を加筆検討
- **既存ユーザーの localStorage 移行(boolean 化対応)**: lively.js の case 'bloomStrength' は
  boolean / number 両対応(`typeof val === 'boolean'` 分岐)。旧バージョン(art-web-settings-v1 に
  bloomStrength キーなし)をロードしても、`webui.js` の loadSaved が欠落キーを no-op としてスキップして
  起動時の Settings.bloomStrength=0 が使われる。旧バージョン(数値 60 等が保存されている遷移期ユーザー)も
  Number 互換分岐で動き、起動時の 1 回だけ古い値で動いて次の save() で boolean に書き換わる。
  両対応を外すと旧データ復元で動作不定 → 過去のクライアントが残っている可能性があるうちは
  両対応を維持する
