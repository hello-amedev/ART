...
### 配布物の構成詰め(直近の優先・2026-06-21 で着手予定)

あめさんの新方針(2026-06-21 セッション):
- ① Web 公開版とは別に **入口ページ無しのダウンロード用ファイル一式**(ローカル + 壁紙共通)を作成
- ② **BOOTH(日本語)・GUMROAD(英語)** で有料販売(3,000 円程度)
- ③ 入口ページからダウンロードページへのリンクを追加(BOOTH / GUMROAD の URL が確定してから)
- 位置づけ: 作品としては Web で完全版が閲覧でき、販売は AI for ART に賛同してくれるなら、程度

直近の次セッションは **① の構成詰め(zip の中身を確定 + 作り方を決める)**。論点:
- **入れるもの**: `night-of-life/index.html` / `LivelyProperties.json` / `js/*`(全 JS ファイル) /
  日本語版 zip = `README.txt`、英語版 zip = `README.en.txt`(言語版で別 zip 配布想定)
- **入れないもの**: `HANDOFF.md`(開発記録)/ `CONCEPT.md`(リポジトリの開発用文書)/ `.git*` /
  その他の開発ファイル
- **zip 名 / バージョニング**: 命名規則は次セッションで決定(例: `night-of-life-v2.0.0.zip` /
  `night-of-life-v2.0.0-en.zip` 等)。今のところ作品本体に書かれているバージョンは v2.0.0
- **作り分けの自動化**: 手動 zip 化 / シェルスクリプト / GitHub Actions のいずれにするか次セッションで
  判断。最初は手動で 2 言語分の zip を作る運用でも当面は OK
- **配布版 index.html は Web 公開版と同一**(分岐ロジック不要)。Web 設定 UI(webui.js)は Lively 環境で
  自動抑制される(5 章の Lively 抑制ロジック)ので、同じ index.html でブラウザ / Lively / 配布 zip
  すべて動く
- **後段**: BOOTH / GUMROAD の URL が確定したら、③ 入口ページにダウンロードリンクを追加
  (`.work` ブロック内、`.enter`(作品を見る →)の隣か下に「ダウンロード版」リンクを置く想定)
