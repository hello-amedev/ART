...
- **Lively 環境では camListeners を全部解除する(2026-06-21 午後)**: livelyPropertyListener が
  呼ばれた瞬間に main.js のラッパーが起動し、`camListeners` 配列の全 addEventListener を
  removeEventListener する。webui.js が Web 設定 UI を Lively で撤去するのと同じパターン。
  ロード順は lively → main → webui なので、Lively からの呼び出しは webui のラッパー → main の
  ラッパー → 本物の lively.js listener の順で連鎖する(両ラッパーが teardown を実行する)。
  デッドゾーンと併用することで「Lively 検出前に偽 pointerdown が来ても、5 px 未満で終わるなら
  視覚影響なし」の二重防御
