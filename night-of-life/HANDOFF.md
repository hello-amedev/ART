...
**既定 ON で起動(二段階デプロイ第 2 段階完了)**。LP value=true / lively.js Settings.bloomStrength=0.5。
webui.js の Defaults は `Settings.bloomStrength > 0.001` で自動連動するので触らない。
既存ユーザー(art-web-settings-v1 に bloomStrength=false 保存済み)は loadSaved で上書きされるので影響を受けず、
新規ユーザー / Lively で初めて壁紙追加した人 / localStorage クリア後の人に ON で見せる。
...
- **オプション撤去 + 50% 固定の判断(検討中)**:
  二段階デプロイ第 2 段階(既定 OFF → ON)は本セッションで完了済み。残るのはあめさん本人の意向で
  「光のにじみのオプションを撤去して 50% 固定にするか」の判断。撤去するなら触る場所:
  ...
  (c) lively.js から case 'bloomStrength' を削除(Settings 初期値 0.5 はそのまま)、
  ...
  撤去しない場合の保留状態(ON/OFF オプション付き・既定 ON)も問題なく動く
