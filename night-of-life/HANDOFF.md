...
> **公開状態(2026-06-23 セッション末): 「生命の夜」を BOOTH と GUMROAD でストア公開済み。入口ページ
> (ART 直下 `index.html`)に作品ボタンの英語併記と各ストアへの配布リンクを追加 → コミット(`98253a5`)→
> main へ ff マージ → `origin` へ push 済み = 本番(GitHub Pages)公開済み。配布 zip は今回の変更で
> 再生成不要(入口ページは zip 内に含まれない)。**
>
> - BOOTH (ja): https://ame-dev.booth.pm/items/8537182
> - GUMROAD (en): https://amedev.gumroad.com/l/anightoflife
>
> (前回 2026-06-22 セッション末: 案A + SSAA + density-contrast を `7056b0e` で公開済み。
> 以下の各記述に残る「コミット前 / 動作確認・コミットは未」は記録時点の表現で、最終的には公開済み)

**2026-06-22(その2): 案A後に残る「密集部のかき傷/色ノイズ」対策 = SSAA + density-contrast**。1080p 実機で案A確認後、
あめさん FB「密集した中〜遠景がかき傷のようにざわつく + 拡大すると色チラつき」を受けて第2弾。多観点ワークフローで
次の一手を比較(遠景ブラー/SSAA/遠景線統合/密度コントラスト/組み合わせ)。**遠景ブラー(被写界深度)は却下**:
現状の描画は残像蓄積バッファ(毎フレーム非clear + 加算合成)なので、単一 FBO では深度分離が原理的に不可能で
近景シャープが壊れる(本格対応は深度を別アタッチメントに出す再設計が必要 = 将来課題)。採用は **SSAA(本命)+
density-contrast(色ノイズ補助)** の2本。SSAA = 内部 FBO を `renderScale` 倍で描き blit で LINEAR 縮小(かき傷・
色ノイズが縮小時の平均で溶ける・近景シャープは構造的に保たれる)。density-contrast = 最奥(dn>0.55)の彩度/明度を
非線形に沈めて色ノイズを抑制。**既定値(= Lively 壁紙もこの値)= ssaa 1.3 / dcSat 0.5 / dcAlpha 0.2**。開発用 URL
`?ssaa=` `?dcsat=` `?dca=` で生調整。**SSAA は塗りコストが倍率²なので、あめさんの 1080p 実機で「ザワつき低減 vs
fps vs オービット微明滅」を確認して最終倍率を確定する段階**(コミット前)。進化・物理は無改修。詳細は 3 章。

**2026-06-22: 低解像度(1080p)での遠景の点々を改善(案A = 最低長の床 + 先端にじみ)**。あめさん実機 FB
「1080p モニタだと奥の光が点々に見える(2K では気にならない)」を受けて対応。多観点ワークフローで原因を確証
(独立 2 診断官 high confidence 一致): 遠景の針は投影で長さが点に潰れる(太さは 1px 下限があるが**長さに
下限なし**)+ 距離 AA が幅方向のみで先端は硬い + 大気遠近減光でブルームも届かない。2K で綺麗なのは Windows
拡大率 125〜150% で実効 DPR が上がり実質スーパーサンプルされるため(投影 CSS-px 長は解像度不変)。対策は案A:
(1) 頂点シェーダで投影された針の**最低長を床で保証**(向きは保持・中心対称に伸ばす)、(2) フラグメントで針の
**先端(長さ方向)を内向きに溶かす AA**を追加。GL 経路が本体、2D 経路には最低長の床のみ鏡像(lineCap=butt は
不変)。既定値は安全側固定(最低長=物理 2.5px 相当・dpr 正規化で高 DPR 機は効きが弱まる / 先端にじみ=1.0px
CSS)。開発用 URL `?ffmin=` `?ffsoft=` で生調整可。**手元 2K では最終値を確定できないため、1080p 実機 FB を
待って固定値を確定する段階**(コミットは動作確認後)。進化・物理は無改修(`art-evolution-v2` 据え置き)。詳細は 3 章。

**2026-06-21: README を購入者向けに .txt 化 + AI for ART コンセプト統一**。本日のセッションで
あめさんから今後の方針(① Web 公開版とは別に入口ページ無しのダウンロード用ファイル一式を作成 /
② BOOTH(日本語)・GUMROAD(英語)で 3,000 円程度の有料販売 / ③ 入口ページからダウンロードページ
へのリンク追加)が提示され、位置づけは「作品としては Web で完全版が閲覧でき、販売は AI for ART
に賛同してくれるなら、程度」。これを受けて配布物の起点となる README を全面刷新。`README.md` /
`README.en.md` を git rm し、配布物としてメモ帳で開かれる前提で **`README.txt` / `README.en.txt`
(UTF-8 BOM なし・CRLF・案 X 書式 = ■ 見出し + コロン整列の中間装飾)** を新規作成
(リポジトリと配布 zip 兼用)。構成は **リード → ステートメント → 鑑賞のはじめかた(ブラウザ +
Lively)→ 観測パネル(OBSERVATORIUM)の見方 → 15 の遺伝子 → クレジット(Partner: Claude Fable 5,
Opus 4.8)→ ライセンス(個人鑑賞のみ・再配布 / 転売 / 改変公開禁止)→ プロジェクト(末尾に
AI for ART コンセプト文の日英)**。同時に **「キャンペーン → プロジェクト」「進化し続ける →
変化し続ける」を全箇所で統一**(入口ページ・CONCEPT.md・README・作品本体 night-of-life/index.html
の SNS シェア meta まで含む)、**ステートメントを差し替え**(「その輝きと、越えていく時間を、」→
「**その軌跡を**」/ 英訳 "its glow and the time it traverses" → **"its trajectory"**)、入口ページ
の work-sub に **英訳併記(改行 + italic)** を追加。**進化・物理(genome/evolution/species/field)・
WebGL2 経路は無改修**。新遺伝子なし = `art-evolution-v2` 据え置き。次は配布物の構成詰め(4 章冒頭)。
詳細は 3 章、新規追加した CSS や落とし穴は 5 章。

**さらに同日**: 配布物のビルドフローも確立。`build-dist.ps1`(+ `build-dist.bat` ラッパー)で
`dist/night-of-life-v2.0.0-ja.zip`(BOOTH 用)と `-en.zip`(GUMROAD 用)が一発で生成される
(各 54KB)。英語版 zip 用に `night-of-life/LivelyProperties.en.json` も新規追加(Lively 設定パネルは
LP の `text` を固定表示する仕様で動的言語切替不可のため、英訳 LP を別途用意して言語別 zip に
同梱する必要があった)。`.gitignore` に `dist/` 追加。手順は 6 章。

--- 以下は 2026-06-20 後段(Step 2-3 + 常時 ON 化)の記録 ---
...
- 2026-06-23 (ストア公開 + 入口ページに配布導線): 本セッション、コミット 98253a5
  - **きっかけ**: あめさんが「生命の夜」を BOOTH と GUMROAD でストア公開完了。
    HANDOFF L62-64 で予告されていた「③ 入口ページからダウンロードページへのリンク追加」を実装
  - **方針**: 入口ページの「美術館の入口」トーンを保つため、配布導線は「作品を見る」ボタンに対して
    控えめなテキストリンクで添える(主従:体験 > 配布)
  - **実装(ART 直下 `index.html`)**:
    - 「作品を見る」ボタンに英語併記 "View the work" を追加(日英をスラッシュで 1 単位に)
    - ボタン直下に新クラス `.download` で控えめなテキスト行を追加:
      `作品をダウンロード / Download the work :　BOOTH ↗   Gumroad ↗`
    - BOOTH / Gumroad の固有名詞だけがリンク(各 1 リンク)、外部リンクなので `target="_blank" rel="noopener"`
    - 視覚: 灰色(`--dim`)・小さめ(12.5px)、リンクは金色の薄い下線、hover で金色
    - レイアウト: `:` の前に半角、後に全角スペース。BOOTH と Gumroad の間は `a + a { margin-left: 16px }`
  - **ストア URL**:
    - BOOTH (ja): https://ame-dev.booth.pm/items/8537182
    - GUMROAD (en): https://amedev.gumroad.com/l/anightoflife
  - **進化・物理・両 renderer・LP・README・作品本体は無改修**。`art-evolution-v2` 据え置き
  - **配布 zip 再生成は不要**(入口ページは zip に含まれない)

- 2026-06-22 (案A後の残課題対策 = SSAA + density-contrast): 本セッション(案A の続き)
  - **きっかけ**: 1080p 実機で案A確認後のあめさん FB「点 → 短い筆致にはなったが、密集した中〜遠景が
    かき傷/毛羽立ちのようにざわつく(細い 1px シャープな短線が高密度に重なる高空間周波数ノイズ)+ 拡大すると
    種族色 + 色相波の細線が混ざる色ズレ/色チラつき」。近景の太く明るい筆致は良好。
  - **多観点ワークフローで次の一手を比較(5レバー → 各案を敵対的検証 → 統合)**:
    - **遠景ブラー(被写界深度)= 却下**。理屈(半解像度ガウシアンで色ズレ混色 + 高周波ノイズを潰す)は正しいが、
      提案された「深度を HDR FBO の alpha チャンネルに畳む」実装が現状コードと矛盾。FBO は毎フレーム clear されず
      (paintFull のみ)、preserveDrawingBuffer + 背景フェード(SRC_ALPHA)で**残像を蓄積する**作りなので、A は
      深度ではなく軌跡の残像になり抽出キーにならない。加えて加算合成では同一ピクセルに近景・遠景が混在し、
      **単一 FBO では深度分離が原理的に不可能** → 近景シャープが壊れる。本格対応は深度を別アタッチメント(MRT)に
      linearDepth として出すか粒子を近/遠2バッファに振り分ける再設計が必要 = **将来課題**(4 章)。
    - **far-merge(遠景線を太く+溶かす)= 補助どまり**。色ノイズに効かず、ゲートに a_lineBase(種族サイズ込み)を
      使うと近景シャープが種族依存で崩れる。採用見送り。
  - **採用 = SSAA(本命)+ density-contrast(色ノイズ補助)**:
    - **SSAA(render-gl.js)**: `renderScale` を導入。`resize()` で HDR FBO とブルーム FBO を **renderScale 倍の内部
      解像度**で作成し、最終 blit で画面サイズ(canvas.width/height)へ **LINEAR 縮小**。縮小時のバイリニア平均で
      かき傷・色ノイズが溶ける。`u_resolution` は CSS px(env.vw/vh)を渡しているので **viewport を倍率倍するだけで
      線幅・案A床は不変 = 真のスーパーサンプリング**(`canvas.width` は変えない)。`_createFBO` の filter を
      NEAREST→LINEAR。`_renderBloom` の texelStep を **×renderScale** して画面上のブルーム半径を倍率不変に保つ
      (これをしないと s を上げるほどブルームが細る)。8-bit 直描画(FBO 無し)では eff=1.0 で等倍。
    - **density-contrast(render-gl.js _packParticles + main.js 2D draw)**: `far = smoothstep(0.55, 1, dn)`(dn<0.55 で
      0 = 近〜中景は不変)を導入し、`sat *= (1 - far*dcSat)` / `alpha *= (1 - far*dcAlpha)`。最奥の彩度を主に落として
      色ノイズを抑え、明度は控えめに。CPU 側 sat/alpha のみ変更でパイプライン無干渉。GL/2D 両方に鏡像。
  - **既定値(= Flags の値 = Lively 壁紙もこの値で動く)**: `ssaa=1.3` / `dcSat=0.5` / `dcAlpha=0.2`。
    開発用 URL `?ssaa=`(1.0〜2.0)/ `?dcsat=`(0〜1)/ `?dca=`(0〜1)で生調整。Flags が単一の真実源、
    constructor は安全 no-op フォールバック(1.0/0/0)。main.js が `glRenderer.renderScale/dcSat/dcAlpha` へ反映
    (**renderScale は resize で FBO 生成時に読むので、必ず初回 resize() = main.js L699 より前の plumbing 位置 L36 で設定**)。
  - **性能**: SSAA は塗りコストが **倍率²**(1.3→1.69倍 / 1.5→2.25倍)。HDR FBO VRAM も倍率²(1080p で 1.3 倍 ≈ 28MB)。
    24/7 iGPU 壁紙で最も重い方向なので、あめさんの 1080p 実機で fps と「オービット中の最密遠景の微明滅(一様明滅
    禁忌に接近)」を確認して最終倍率を確定する。許容ラインは**バランス(〜1.3倍目安)**(あめさん)。
  - **進化・物理(genome/evolution/species/field)・ブルーム本体・案A は無改修**。`art-evolution-v2` 据え置き。
  - **再ビルド必要**: `build-dist` で配布 zip を再生成。**構文チェック済み**(node --check OK)。**動作確認・コミットは未**。

- 2026-06-22 (遠景の点々を改善 = 案A: 最低長の床 + 先端にじみ): 本セッション
  - **きっかけ**: あめさん実機 FB「開発は 2K でやっていて気づかなかったが、1080p モニタだと奥の光が
    点々(散らばった硬い点)に見える」。
  - **原因(多観点ワークフロー + 独立 2 診断官 high confidence 一致)**:
    (1) 遠景の針は投影で長さが点に潰れる。`render-gl.js` PARTICLE_VERT は太さに max(1.0,...) の 1px 床が
        あるが**投影長 alen には下限が無い**。1080p 最奥級で長さ約 1.4〜2.3px × 幅 1px のほぼ点になる。
    (2) 距離 AA(PARTICLE_FRAG)は **perp(幅)方向のみ**。針の先端(長さ方向)は AA されず硬い。
    (3) 大気遠近減光で遠景 alpha/lum が下がり、ブルーム閾値(Rec.709 輝度 > 1.0)に届かず平滑化も来ない。
    (4) 1080p デスクトップは DPR=1 で `dpr=min(1,1.5)=1` → スーパーサンプルされない。**dprCap を上げても
        min で頭打ちのため DPR=1 では無効**。
    - 重要: strokeLen は world 長で worldR/camDist/focal が全て base(解像度)に比例 → **投影 CSS-px 長は
      解像度不変**(最奥 strokeLen=7 は 1080p でも 2K でも約 1.94 CSS px)。2K で綺麗なのは Windows 拡大率
      125〜150% で実効 DPR≈1.25〜1.5 になり実質 SS されるため。差は「投影長」ではなく「device pixel 密度」。
  - **採用案 = 案A(minlen + softends を 1 セット)**。検証で haze(間引き)は survives=false(点の硬さを
    直さない・間引きは症状と逆向き)、SSAA(案C)は塗り 2.25 倍で 24/7 iGPU 壁紙に最も重く根本(幾何)も
    直さないため、案A を主軸に採用。各レバー単独だと弱点(minlen は先端が硬いまま / softends は点が点のまま)が
    残るので 1 セットで実装。
  - **GL 経路(render-gl.js)**:
    - 頂点シェーダ: `u_minHalfLen` 追加。`halfLen = max(0.5*alen, u_minHalfLen)` で最低長を床。端点を
      A.xy/B.xy 直接ではなく `center ± dir*halfLen` に変更(床が効かない時は 0.5*alen=端点と一致 → **近景は
      完全に不変**)。varying `v_alongNorm`(-1..+1)/ `v_halfLen` を追加。
    - フラグメントシェーダ: `u_endFade` 追加。`aaL = 1 - smoothstep(halfLen-endFade, halfLen, |alongNorm|*halfLen)`
      で先端を内向きに溶かし、既存の幅 AA(aaW)と乗算。**固定 1px の先端フェードは長い近景の針では相対的に
      無視でき、点に潰れた遠景の針でだけ大きく効く**(自然に遠景限定)。
    - `u_minHalfLen` は draw() で `(farMinLenPhys / dprEff) * 0.5`(CSS px 半長)を渡す。シェーダーは
      u_resolution=CSS px 空間で動くので、物理 px 床を dpr で割って CSS px 床に換算 = **高 DPR 機では床が
      自動的に小さくなり効きが弱まる**(2K/4K = あめさんの手元 に余計な変化を出さない)。
    - constructor に既定値 `farMinLenPhys=2.5` / `farEndFadeCss=1.0`、init() で uniform location 取得。
  - **2D 経路(main.js draw)**: 最低長の床だけ鏡像(`minLen2d = farMinLenPhys / dpr2d`、中心対称に伸ばす)。
    **先端処理・lineCap(butt)は触らない**(丸い先端=頭=生物感。2D はブラウザが線を AA するので床だけで足りる)。
  - **開発用 URL パラメータ(lively.js Flags)**: `?ffmin=`(最低長・物理px・0〜3.0 クランプ・既定 2.5)、
    `?ffsoft=`(先端にじみ・CSS px・0〜3.0 クランプ・既定 1.0)。既存の `?bloom=0` と同じ隠しフラグ方式で
    UI には出さない。main.js が `glRenderer.farMinLenPhys / farEndFadeCss` へ反映(killBloom/bloomDebug と同列)。
  - **既定値の根拠**: 手元 2K では最終値を確定できないため、低リスクな安全側固定値で先行する。先端にじみは
    硬さを和らげるだけ / 最低長は最も潰れた針だけ少し伸ばすだけ + dpr 正規化で 2K(手元)はほぼ変化なし=
    手元の絵を壊さない。**1080p 実機 FB を待って `?ffmin=` `?ffsoft=` で詰め、固定値を確定**。
  - **進化・物理(genome/evolution/species/field)・ブルーム・大気遠近・dragBoost は無改修**。
    `art-evolution-v2` 据え置き = リセット不要。
  - **再ビルド必要**: `build-dist` で配布 zip を再生成(zip 内 index.html 経由で js が同梱されるため)。
  - **構文チェック済み**(`node --check` で render-gl.js / main.js / lively.js OK)。**動作確認・コミットは未**。

- 2026-06-21 (README 購入者向け刷新 + AI for ART コンセプト統一): 本セッション、コミット 07726e5
  - **きっかけ**: あめさんが今後の方針として「① Web 公開版とは別に入口ページ無しのダウンロード用
    ファイル一式(ローカル + 壁紙共通)を作成、② BOOTH(日本語)・GUMROAD(英語)等で 3,000 円程度の
    有料販売、③ 入口ページからダウンロードページへのリンク追加」を提示。位置づけは「作品としては
    Web で完全版が閲覧でき、販売は AI for ART に賛同してくれるなら、程度」。この方針を踏まえて、
    まず配布物の起点となる README から手を入れた
  - **配布形式の変更(.md → .txt)**: 配布物はメモ帳で開かれることが多く、Markdown 記法
    (`# ` 見出し / `| 表 |` / `**bold**` / バッククォート)が剥き出しに見えて汚い。
    `night-of-life/README.md` と `README.en.md` を git rm、新規に **`README.txt` / `README.en.txt`**
    を作成。書式は **案 X(中間装飾)** = `==========` でタイトル区切り / `■` 見出し /
    リスト `- 項目` / 表は `項目 : 説明` のコロン整列。文字コードは **UTF-8 BOM なし**、改行は
    **CRLF**(Windows メモ帳前提)。Write 直後は LF だったので
    `sed -i 's/\r$//; s/$/\r/' file` で CRLF 化 → `file` コマンドで「Unicode text, UTF-8 text,
    with CRLF line terminators」を確認(BOM は付かない)。リポジトリ側の `.md` も削除して
    **`.txt` 一本化**(配布 zip と GitHub リポジトリの README を兼用)
  - **README の新構成**(購入者 = アート作品の鑑賞者を想定):
    - 表題 + リード「遺伝的アルゴリズムで変化し続ける光のアート」
    - ステートメント(入口ページから丸ごと引用 = 同文・修正後)
    - 鑑賞のはじめかた = (a) ブラウザで開く `index.html` ダブルクリック・インターネット接続不要、
      (b) Lively Wallpaper(無料の壁紙ソフト)に対応 + 公式 URL 1 行(あめさん指示「Lively は他人の
      製品なので適正な範囲で紹介」= 推奨表現を避けて客観的な紹介に留める)
    - 観測パネル(OBSERVATORIUM)の見方 = 色チップ / gen / from / act / vig / spd / flk / len / age /
      状態 の 10 列をコロン整列で。状態の凡例 `· 通常 / in 参入 / ↓out 退場 / ✦ 突然変異` も同行で
    - **15 の遺伝子** = genome.js の `GENES` 定義から **色彩 6 / 動き 3 / 群れ 4 / 時間帯 2** を
      4 カテゴリ `[色彩] [動き] [群れ] [時間帯]` で整理。各遺伝子に **英識別子 + 数値範囲 + 一言説明**
      を併記(例:`hueOffset    基調色相からのずれ(-110〜+110°)`)。あめさん指示「『隠れている』は
      情緒的すぎ」を反映し見出しは「**15 の遺伝子**」とフラットに
    - クレジット(制作: ame_dev / Partner: Claude Fable 5, Opus 4.8)
    - ライセンス(**個人鑑賞のみ可。再配布・転売・改変しての公開を禁止します。**)。あめさん明示
      「再配布禁止は必ず入れる」/ 改変は「私的改変は黙認、公開はダメ」のスタンス
    - プロジェクト = `AI for ART` 表示 + 入口ページ URL `https://hello-amedev.github.io/ART/` +
      末尾に AI for ART コンセプト文を **日英で追加**(入口ページの lead 文をそのまま引用・キャンペーン
      → プロジェクトに置換した版)
  - **キャンペーン → プロジェクト の表記統一**(あめさん指示「AI for ART は『キャンペーン』ではなく
    『プロジェクト』」): 入口ページ meta description / `/* イントロ(キャンペーンの趣旨) */` CSS コメント /
    lead 日 / lead 英(`personal campaign` → `personal project`)/ CONCEPT.md 見出し
    `## キャンペーンの趣旨` → `## プロジェクトの趣旨` / 太字 lead 末尾「個人的なキャンペーン。」/
    L18 「反転がこのキャンペーン。」の **全 7 箇所** を一括変更。HANDOFF.md 内のキャンペーン記述
    (L442 / L465)は **過去記録なので触らない**(歴史を保存)
  - **進化し続ける → 変化し続ける の表記統一**: 新 README で「変化し続ける」を採用したことで表記揺れが
    発生(入口ページは「進化」)。grep で発見し、入口ページ meta description / work-sub / 作品本体
    `night-of-life/index.html` の meta description / og:description / twitter:description の **全 6 箇所**
    を「変化」に統一(あめさん判断「全部統一」)。HANDOFF.md 内の「進化」記述は **過去記録なので
    触らない**
  - **ステートメント差し替え(その軌跡を / its trajectory)**: 入口ページ・CONCEPT.md・README.txt の
    3 箇所(日)+ 入口ページ・README.en.txt の 2 箇所(英)。日本語「その輝きと、越えていく時間を、
    デジタルアートとして表出させることを試みた」→「**その軌跡を、**デジタルアートとして表出させる
    ことを試みた」、英訳 "This is an attempt to express, as digital art, its glow and the time it
    traverses." → **"This is an attempt to express its trajectory as digital art."**。「軌跡」の英訳は
    3 案(trajectory / trail / passage)からあめさん選択で **trajectory** 確定(物理学的・哲学的な
    響きが原文の「絶えることなく『夜』を渡っていくプロジェクト」の重さに合うため)
  - **入口ページ work-sub に英訳併記**: 「遺伝的アルゴリズムで変化し続ける光のアート」の英訳
    "Light art that keeps changing under a genetic algorithm" を併記。work-title と同じ
    `<span class="en">` パターン + **改行(`<br>`)** で日英を縦に並べる(あめさん指示で
    最初は同じ行で書いたが「英訳の前に改行」と修正)。CSS に
    `.work-sub .en { font-style: italic; color: rgba(255, 255, 255, 0.36); }` を追加して英文は
    斜体 + 一段薄い色で日本語と視覚的に区別(work-title の en と同じトーン)
  - **新ステートメントの英訳 = 案 A 採用**: 本文の英訳 3 案(literary 寄り / 平明 / 詩的)から、
    入口ページの旧英訳(`accidental / inevitable / left to the observer`)と語彙が連続する **案 A**
    を選択。入口ページと配布版 README を比較した時に違和感がない
  - **CSS 追加(.work-sub .en)**: 入口ページ index.html L60 直下に
    `.work-sub .en { font-style: italic; color: rgba(255, 255, 255, 0.36); }` を追加。work-title の
    `.en`(`font-size:14px; color: var(--dim); letter-spacing:0.08em;`)は work-title 専用セレクタ
    (`.work-title .en`)で work-sub には適用されないため、別途定義
  - **進化・物理(genome / evolution / species / field)・WebGL2 経路は無改修**。`art-evolution-v2`
    据え置き = リセット不要

- 2026-06-21 (配布物のビルドフロー確立 + 英語版 LP): 本セッション
  - **きっかけ**: README 刷新(コミット 07726e5)に続いて配布物の構成詰めへ。HANDOFF 4 章で
    「壁紙の配布」を未着手としていたものを実装フェーズへ
  - **論点 A〜D の確定**: ① 言語別 = 2 zip(BOOTH = ja / GUMROAD = en)、② フォルダあり解凍
    (`night-of-life-v2.0.0-ja/` フォルダができる、デスクトップ散乱回避)、③ zip 名 =
    `night-of-life-vX.Y.Z-{ja|en}.zip`(`v` 付き・GitHub Releases 慣習)、④ 作り方 = スクリプト
    自動化(コマンド一発で 2 zip 生成・README 入れ忘れ等の手動ミスを防ぐ)
  - **PowerShell 採用の経緯**: 当初は bash `build-dist.sh` で書いたが、Git Bash on Windows に
    `zip` コマンドが含まれず実行不可。PowerShell の `Compress-Archive`(Windows 標準・追加
    インストール不要)でネイティブ完結する形に切り替え(`.sh` は削除済み)。`build-dist.bat`
    ラッパー(`powershell -NoProfile -ExecutionPolicy Bypass -File build-dist.ps1` を呼ぶ)を
    同梱して、ダブルクリックでビルドできるようにした。スクリプトは `-Force` フラグや事前の
    stage クリーン化(`if (Test-Path $stage) { Remove-Item -Recurse -Force $stage }`)で、
    前回失敗の残骸があっても再実行で必ずクリーンに作り直せる堅牢性を確保
  - **英語版 LP の必要性**: Lively は LP の `text` フィールドを固定表示する仕様(動的言語切替の仕組み
    なし)。英語版 zip に日本語 LP を入れると Lively 設定パネルが日本語のまま出てしまうので、
    `night-of-life/LivelyProperties.en.json` を新規作成(全 10 項目の英訳・**webui.js の英訳
    テーブルと完全一致**)。`build-dist.ps1` の英語版ビルド時に配布 zip 内で
    `LivelyProperties.json` にリネームして同梱する形
  - **配布 zip の中身**(12 ファイル / 約 54KB):
    - `index.html` / `LivelyProperties.json` / `js/*`(9 ファイル) / `README.txt`(言語別)
    - **除外**: `HANDOFF.md` / `CONCEPT.md` / `.git*` / 開発用ファイル
    - 配布版 `index.html` は Web 公開版と完全同一(分岐ロジック不要)。webui.js は Lively 環境で
      自動抑制される(5 章の Lively 抑制ロジック)ので、ブラウザ / Lively / 配布 zip すべて同じ
      ファイルで動く
  - **`.gitignore` 更新**: `dist/` を追加(配布 zip は git 管理外)
  - **動作確認済み**: PowerShell スクリプト実行 → zip 生成成功 → Expand-Archive で両 zip 解凍 →
    ファイル一覧確認(12 ファイル / フォルダ付き)→ 各 LP の `text` 行が日本語 / 英語に正しく
    分かれていることを確認。`README.txt` の冒頭も言語ごとに ja=「生命の夜 — A Night of Life」/
    en=「A Night of Life — 生命の夜」と確認

- 2026-06-21 (壁紙でのカメラ操作誤発火対策): 本セッション午後の実機 FB
  - **症状**: あめさんが配布版を Lively 壁紙にして検証したところ、カメラ移動自体はできないが
    「クリックを微妙に拾ってしまい、視点回転時の paintFull / dragBoost=15 が一瞬走って光の尾が
    瞬間的に消える」現象が発生
  - **原因**: pointerdown だけで即 dragBoost / paintFull / setPointerCapture を起動していたため、
    Lively 環境で何らかの偽 pointerdown が発火した時に視覚効果が一瞬出てしまう
  - **対処 = 案 C(B + C 組み合わせ)**(あめさん当初案の「配布版を別ファイル化」は HANDOFF
    L587-589 で確立した「配布版 index.html は Web 公開版と同一」方針とぶつかるため不採用):
    - **(B) Lively 環境検出 → listener 解除**: main.js で `livelyPropertyListener` をラップ
      (webui.js が Web 設定 UI を Lively で撤去するのと同じパターン)。Lively が呼んだ瞬間に
      `camListeners` 配列の全 addEventListener を removeEventListener する。ロード順は
      lively → main → webui で、webui.js も別途同じラッパーを掛けるが、両ラッパーが連鎖して
      呼ばれるので両方とも teardown される
    - **(C) デッドゾーン 5 px**: `camDragPending` 候補状態を新規導入し、pointerdown では起動せず
      候補のみ。pointermove で 5 px 以上動いたら確定(`confirmCamDrag` 呼び出しで dragBoost=15
      + setPointerCapture)。クリックや 5 px 未満の微小動きでは視覚効果ゼロ(候補状態は
      dragBoost も paintFull も触らない)
    - **確定時の座標基準**: startX/Y からの累積で動かすと最初の 1 フレームで dead-zone ぶん
      カメラがジャンプするので、確定時点の座標を `camDrag.lastX/Y` の初期値にする
  - **index.html は 1 つのまま**: 案 C なら Web/Lively 両環境とも適切に動く(Lively では完全
    無効化、Web では誤クリックも吸収する保険として残る)。HANDOFF 4 章「配布物のビルドフロー」
    方針は維持
  - **再ビルド必要**: ART メインで `build-dist.bat` を再実行して `dist/*.zip` を新規生成する
    必要あり(zip 内の `index.html` 経由で `js/main.js` が同梱されるため、main.js を更新したら
    zip も作り直す)
  - **進化・物理・両 renderer・LP・README は無改修**
  - **続いて修正(2026-06-21 午後 2 回目)**: 初版は main.js が直接 `livelyPropertyListener` を
    ラップする実装だったが、webui.js が起動時に `realListener = window.livelyPropertyListener` を
    保持し設定変更時に `realListener.apply()` で呼ぶため、Web UI からの設定操作で main.js の
    ラッパーが起動 → ローカル環境でもカメラ listener が解除されてしまう問題が発覚(あめさん
    実機 FB「ローカルでカメラ操作できない」)。修正: main.js から `livelyPropertyListener` ラップを
    撤去し、代わりに `window.__nolCameraTeardown` 関数を公開。webui.js の既存 Lively 検出フックから
    呼んでもらう形に変更(Lively 検出ロジックを webui.js 1 箇所に集約)。Web UI からの設定変更は
    realListener(= lively.js の本物)を直接呼ぶので main.js のフックを通らず、ローカル環境では
    カメラ操作が正常に動く。Lively 環境では webui.js が `livelyPropertyListener` を捕捉した時に
    `window.__nolCameraTeardown()` も呼ばれてカメラも teardown される。これが確定実装
...
### 遠景のなめらか化(案A + SSAA + density-contrast)の 1080p 実機チューニング = 着手中(2026-06-22)

案A(最低長 + 先端にじみ)+ 第2弾(SSAA + density-contrast)を実装済み(3 章)。**全ツマミと既定値**:
- `?ffmin`(案A 最低長・物理px・既定 2.5・0〜3.0)/ `?ffsoft`(案A 先端にじみ・CSS px・既定 1.0・0〜3.0)
- `?ssaa`(SSAA 倍率・既定 1.3・1.0〜2.0)/ `?dcsat`(最奥の彩度沈め・既定 0.5・0〜1)/ `?dca`(最奥の明度沈め・既定 0.2・0〜1)

次のステップ(1080p 実機で目視 → 最良値を確定 → constructor / Flags 既定値を更新):
- **SSAA**: `?ssaa=1.0`(=現状)→ 1.25 → 1.3 → 1.5 と振り、「かき傷の溶け具合」「fps が保てるか」
  「**オービット中に最密遠景が微明滅しないか**(一様明滅禁忌に接近)」を確認。許容ラインはバランス(〜1.3 目安)。
  微明滅 or カクつきが出たら倍率を下げる(明滅ゼロ優先)。最終値を Flags `ssaa` 既定に焼き込む。
- **density-contrast**: `?dcsat=` で色ノイズの底を調整(0.5 から)。`?dca=` は控えめ(0.2)。上げすぎると**渦の最深部が
  空虚化(ホロウ化)**して奥行きが痩せるので注意。`?dcsat=0&dca=0` で density を完全 OFF にして比較できる。
- **比較レシピ**: 現状=`?ssaa=1&dcsat=0&dca=0` / SSAA のみ=`?ssaa=1.3&dcsat=0&dca=0` / density のみ=`?ssaa=1&dcsat=0.5&dca=0.2`。
- 案A 側の微調整: 真正面向き(放射方向)の針は床を取りすぎると向きがチラつく恐れ。やりすぎると規則的な粒の絨毯に。

### 遠景の被写界深度ブラー(本格版)= 将来課題(2026-06-22 に dof-blur 却下)

「奥だけぼかして溶かす」遠景ブラーは方向性は正しいが、現状の描画(残像蓄積バッファ = 毎フレーム非clear +
加算合成)では**単一 FBO で深度分離が原理的に不可能**で近景シャープが壊れるため、軽量実装は却下した(3 章)。
本格対応するなら、**深度を別アタッチメント(MRT で linearDepth を出す)** か **粒子を近/遠 2 バッファに振り分けて
遠景のみ別 FBO でぼかす** 再設計が要る(追加 FBO + パス、ブルーム流用の「最小コスト」前提は崩れる)。SSAA +
density で不足を感じた場合の本命候補。要方針確認。



`build-dist.ps1`(+ `build-dist.bat` ラッパー)+ 英語版 LP `LivelyProperties.en.json` を整備して、
配布 zip(BOOTH 用 ja / GUMROAD 用 en)が **コマンド一発(または .bat ダブルクリック)で生成できる
状態**になった。詳細は 3 章、ビルド手順は 6 章。

**確定した方針**:
- ① 配布物 = `night-of-life-v2.0.0-ja.zip` / `-en.zip` の 2 zip(BOOTH = ja / GUMROAD = en)。
  解凍するとフォルダが出来てその中に全ファイル
- ② 価格 3,000 円程度。位置づけは「作品としては Web で完全版が閲覧でき、販売は AI for ART に
  賛同してくれるなら、程度」
- ③ 入口ページからダウンロードリンク = **未着手**(BOOTH / GUMROAD の URL 確定後)

**次のステップ(別セッション)**:
- **配布物の実機検証**: 生成 zip を別フォルダに解凍 → ブラウザで `index.html` を開く + Lively で
  読み込んで設定パネルが想定言語(ja/en それぞれ)で出るかを実機で確認(あめさん)
- **③ 入口ページのダウンロードリンク追加**: BOOTH / GUMROAD のページ URL が確定後、ART 直下の
  `index.html` の `.work` ブロック内に「ダウンロード版」リンクを置く
- **販売ページ整備**: BOOTH / GUMROAD への登録(あめさん側)。商品説明文・スクリーンショット等
- **バージョン上げ時の運用**: `build-dist.ps1` 冒頭の `$Version` リテラルを書き換えて再実行
  (将来的に v2.1.0 等にした場合)。同時に作品本体の OBSERVATORIUM メタ行 v2.0.0 表記
  (`night-of-life/index.html` 内)も同期して上げること(6 章既知)
...
- **配布物の作成(BOOTH / GUMROAD)**: 詳細は冒頭「配布物の構成詰め」を参照。
  あめさん新方針(2026-06-21)で BOOTH(日本語)・GUMROAD(英語)等で 3,000 円程度の有料販売予定。
  配布 zip は **入口ページ無し**(`index.html` = `night-of-life/index.html` のみ)で、ローカル鑑賞 +
  Lively 壁紙の両方で動く同じファイルを同梱。README は .txt 化済み(コミット 07726e5)
...
- **デッドゾーン 5 px(`CAM_DRAG_DEAD_ZONE`)を入れた経緯(2026-06-21 午後)**: 当初は pointerdown
  で即 dragBoost=15 + setPointerCapture が起きる素朴な実装だった。Lively 壁紙にして実機検証したら
  「クリックを微妙に拾って視点回転時の paintFull / dragBoost が一瞬走り、光の尾が瞬間消える」
  問題が発生(あめさん FB)。pointerdown では起動せず、pointermove で 5 px 以上動いてから確定する
  `camDragPending` 候補状態を導入して根本対策。クリックや 5 px 未満の動きは視覚的影響ゼロ。
  値を小さくしすぎると元の問題が再発、大きくしすぎるとドラッグ開始の手応えが悪い(5 px は
  Windows のデフォルトドラッグ閾値と同等)
- **Lively 環境では camListeners を全部解除する(2026-06-21 午後・修正後 = 確定実装)**:
  main.js は `window.__nolCameraTeardown` 関数を公開、webui.js の既存 Lively 検出フック
  (`livelyPropertyListener` ラッパー)から呼んでもらう。Lively 検出ロジックを webui.js 1 箇所に
  集約することで、Web UI からの設定変更で main.js のラッパーが起動してローカル環境のカメラまで
  死ぬループ(初版実装の不具合・実機 FB 発覚)を回避。ロード順は lively → main → webui で、
  main.js は **`livelyPropertyListener` をラップしない**。webui.js が `realListener =
  window.livelyPropertyListener` で保持するのが純粋な lively.js 本物の listener になり、
  Web UI からの設定変更は main.js のフックを通らない。デッドゾーンと併用することで
  「Lively 検出前に偽 pointerdown が来ても、5 px 未満で終わるなら視覚影響なし」の二重防御
- **`confirmCamDrag` 時に startX/Y でなく確定時点座標を基準にする**: 候補状態の startX/Y からの
  累積で動かすと、デッドゾーンを越えた瞬間に最初の 1 フレームで dead-zone ぶん(5+ px)カメラが
  ジャンプする。確定時点の座標を `camDrag.lastX/Y` の初期値にすることでジャンプを回避
...
壁紙データの別途配布(BOOTH / GUMROAD の zip)は **完了(2026-06-21)**。下記「配布物 zip の生成」を参照。

### 配布物 zip の生成(BOOTH / GUMROAD 用)

ART 直下の `build-dist.bat` をダブルクリック(または PowerShell で
`powershell -ExecutionPolicy Bypass -File build-dist.ps1`)。`dist/` に
`night-of-life-v2.0.0-ja.zip`(BOOTH 用・54KB)と `night-of-life-v2.0.0-en.zip`(GUMROAD 用・54KB)が
生成される。

- **共通同梱**: `index.html` / `LivelyProperties.json` / `js/*`(9 ファイル)
- **言語別 README**: `README.txt`(日本語版) / `README.en.txt`(英語版)→ 配布 zip 内では
  両方とも `README.txt` にリネームして同梱
- **言語別 LP**: `LivelyProperties.json`(日本語版) / `LivelyProperties.en.json`(英語版)→
  配布 zip 内では両方とも `LivelyProperties.json` にリネームして同梱
  (Lively 設定パネルが言語ごとに正しいラベルで表示される。LP の `text` は固定文字列で動的言語切替不可)
- **除外**: `HANDOFF.md` / `CONCEPT.md` / 開発用ファイル
- **解凍時のフォルダ構造**: zip 解凍すると `night-of-life-v2.0.0-{ja|en}/` フォルダができ、その中に
  全ファイルが入る(デスクトップ散乱回避)

**バージョン変更**: `build-dist.ps1` 冒頭の `$Version` リテラルを書き換え。zip 名のみ変わる。
配布物の中身バージョンを変える時は別途 OBSERVATORIUM メタ行の v2.0.0 表記
(`night-of-life/index.html` 内)も同期して上げること。

**前提**: Windows 環境(PowerShell)。`Compress-Archive` は標準コマンドで追加インストール不要。
Git Bash on Windows には `zip` コマンドが含まれないので、bash シェルスクリプトでなく PowerShell
スクリプトを採用した経緯がある(3 章 2026-06-21 エントリ参照)。
