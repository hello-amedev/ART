...
**2026-06-20: WebGL2 化 Step 2-2 (ブルーム) 完成**。あめさんと多観点 workflow で設計案 4 案を比較検証し、
採用案「**軽量 Bright pass + 半解像度 1 段の 9-tap 分離ガウシアン**」(設計案 C 発展形)を実装。
...
**既定 0%(完全 OFF)で起動する opt-in**(あめさん視認確定後の二段階デプロイ。LP / lively / webui /
Defaults すべて 0)。**オービット中(env.dragBoost=15)は effectiveStrength = Settings.bloomStrength /
...
`art-evolution-v2` 据え置き。webui の Defaults snapshot に `bloomStrength` が加わったので
`art-web-settings-v1` には bloom 値も保存されるが、旧クライアントとの互換は壊れない(欠落値は 0 で復元)。
詳細は 3 章、落とし穴は 5 章。
...
- **既定値の二段階デプロイ(最優先)**: Settings.bloomStrength の既定値を **0 → 40-60 へ動かす**。
  あめさんが実機放置(2-3 分)で「画面全体の一斉発光」「白飛び大輪」が出ないことを確認したら、
  `js/lively.js` Settings / `js/webui.js` Defaults / `LivelyProperties.json` value の **3 箇所を同期更新**。
  既存ユーザー(`art-web-settings-v1` に bloomStrength=0 が保存されている)は影響を受けないが、
  新規ユーザー / Lively で初めて壁紙追加した人には ON 状態で見せる
...
- **既定値 0% (opt-in) を外す時の二段階デプロイ**: 既存ユーザーは `art-web-settings-v1` に
  bloomStrength=0 が保存されているので、`js/lively.js` の Settings 初期値を上げても loadSaved で
  上書きされて影響を受けない。Lively で初めて壁紙追加した新規ユーザーや、localStorage クリア後の
  ユーザーには新既定値が反映される。3 ファイル(lively.js / webui.js Defaults / LivelyProperties.json)
  を同時更新しないと、片方だけ反映される不整合が出る
...
- **Canvas 2D 経路 (`?gl=0`) との視覚差**: 2D 経路はブルーム未実装(設計時点で意図的に WebGL2 限定機能)。
  ...
  1 行を加筆検討
- **既存ユーザーの localStorage 移行**: 旧バージョン(`art-web-settings-v1` に bloomStrength キーなし)
  をロードしても、`webui.js` の loadSaved が欠落キーを no-op としてスキップするので、起動時の Settings.bloomStrength=0 が
  そのまま使われる。クラッシュなし。次回 save で bloomStrength=0 が JSON に書き込まれる
