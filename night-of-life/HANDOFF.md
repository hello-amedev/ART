...
`?gl=1` を付けて起動した時だけ WebGL2 経路に切り替わり、未指定は従来の Canvas 2D が無改修で動く。
**HDR FBO (RGBA16F) を前倒し導入**して 8-bit 量子化由来の残光固着を回避、距離ベース AA で MSAA 喪失を補償、
...
- 2026-06-17 (WebGL2 化 Step 2-1): 本セッション
  ...
  - **`?gl=1` 切替**: main.js で URL フラグ判定し、WebGL2 取得成功時のみ glRenderer 経路。失敗・未指定時は
    従来 Canvas 2D が無改修で動く(paintFull/draw/sweepResidue/resize に glRenderer 早期 return を追加)。
    `index.html` で render-gl.js を main.js より前に load。
  - **HDR FBO (RGBA16F) 前倒し導入**: 描画は内部 16-bit float の FBO へ → 最終 blit パスで 1 度だけ 8-bit 量子化。
...
- **FB-5: ウィンドウ縮小 → 拡大で粒子が中央集中**(エッジケース許容)。**カメラズーム設定**の追加検討:
  `camDist` を倍率で 0.7〜1.6× 動かせるスライダーを Lively / Web UI に追加。引きの絵を試せる遊び場兼用。
  `LivelyProperties.json` + `lively.js` の listener + `main.js` の resize() で `env.camDist *= zoomFactor` を入れる程度。
- **FB-2: 細線の階段ジャギーは 2D 版でも残る**。GL 側は距離ベース AA で改善済みだが、2D 版は Canvas rasterizer 任せ。
  ...
  解消する。
- **README.md / README.en.md の更新**: WebGL 関連は Step 2-2 が完了して既定化される時にまとめて反映する。
  Step 2-1 は実験フラグ(?gl=1)なので README には載せない方針。
...
- **`?gl=1` URL フラグでのみ有効**。Lively からは現状トグルできない(Step 2-2 で既定化検討。
  Lively Properties に GL toggle 追加するなら `LivelyProperties.json` + `lively.js` リスナー追加)。
...
- **カメラの動き ON 時、粒子の流れが斜めに引かれて見える錯視**(あめさん発見)。
  `Settings.cameraMotion = true` だと約 8.7 分周期で azimuth が回転 + 仰角ゆらぎ。粒子の流れ自体は正常だが、
  カメラの回転と流れの相対運動が脳内で「斜めに引かれている」錯視を生む。既定 OFF なので運用では出ない。
  根本対策案:周回速度を更に遅く(`c.az += dt * 0.006` くらい)/ 仰角を止める(`c.el` を初期値固定)/
  カメラ ON 時は粒子の `flowObedience` を一時的に上げる。Step 2-2 とは独立した別タスクで触る予定。
...
- **カメラは既定で固定**(cameraMotion=false)。動きを全部「流れ」にして「カメラか流れか分からない」を回避(あめさん要望)。
